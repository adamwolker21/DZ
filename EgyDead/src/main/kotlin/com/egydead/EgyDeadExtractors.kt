    ...
    override suspend fun getUrl(url: String, referer: String?, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit) {
        ...
        for (host in potentialHosts) {
            ...
            try {
                ...
                if (m3u8Link != null) {
                    // الاستخدام الصحيح لـ newExtractorLink
                    callback(
                        newExtractorLink(
                            source = this.name,
                            name = this.name,
                            url = m3u8Link,
                            type = ExtractorLinkType.M3U8 // تحديد النوع كـ M3U8
                        ) {
                            // استخدام initializer block لتعيين الخصائص الإضافية
                            this.referer = finalPageUrl
                            this.quality = Qualities.Unknown.value
                            // isM3u8 لا حاجة لها لأننا حددنا type = M3U8
                        }
                    )
                    return
                }
